...
- Do not send `tool_key`; agent-gateway generates it.
...
- Do not send `skill_key`; agent-gateway generates it.
...
- Do not send `agent_key`; agent-gateway generates it.
...
- Agent id/key: normally `owner_id:name:version`.
- Names should be stable `snake_case`.
- Keys are generated by agent-gateway. Do not send `tool_key`, `skill_key`, or `agent_key`; provide canonical `provider`, `name`, and `version` inputs instead. Do not keep recovery/import suffixes such as `_restored`, `_backup`, `_copy`, timestamps, or random migration markers in `id` or `name`.
...
- Do not add `tool_key`, `skill_key`, or `agent_key` to register payloads. Those fields are owned by agent-gateway and generated with its canonical rules.
...
- Gateway generates both `tool_key` and concise-register `runtime_id` from canonical `provider:name:version`.
...
- Do not send `tool_key`; gateway rejects it on `/v1/tools/register`.
...
Use with `tool register` to create if the payload includes low-level trigger fields, or with `tool update`. Do not include `tool_key`; gateway generates it from provider/name/runtime version.
...
- `version` defaults to `v1`; `provider` defaults to `internal`; `display_name` defaults to `name`; `category` defaults to `general`; gateway generates `skill_key` from `provider:name:version`.
...
- Do not send `skill_key`; gateway rejects it on `/v1/skills/register`.
...
Use with `skill register` to create if the payload includes low-level trigger fields, or with `skill update`. Do not include `skill_key`; gateway generates it from provider/name/manifest version.
...
- `version` defaults to `v1`; gateway generates `agent_key` from `owner_id:name:version`; `owner_id` defaults to `internal`.
- Do not send `id` or `agent_key` for new concise agent registrations. Reject or normalize names like `react_game_generator_agent_013919`; use canonical `name: "react_game_generator_agent"` plus an intentional `owner_id` and `version`.
...
Use with `agent register` to create if the payload includes low-level trigger fields, or with `agent update`. Do not include `agent_key`; gateway generates it from `owner_id:name:v1` for low-level create and from `owner_id:name:version` for concise register.
